...
interface RecipeDao {
    ...
    suspend fun searchRecipes(name: String): LiveData<List<Recipe>>

    @Query("SELECT * FROM recipe WHERE id = :id")
    suspend fun getRecipe(id: Int): LiveData<Recipe>

    @Query("SELECT * FROM recipe WHERE categoryId = :categoryId")
    suspend fun getRecipesByCategory(categoryId: Int): LiveData<List<Recipe>>

    @Query("SELECT * FROM recipe WHERE isFavorite = 1")
    suspend fun getFavorites(): LiveData<List<Recipe>>
}